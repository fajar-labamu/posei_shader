...
class CompareWidget extends StatefulWidget {
  ...
  @override
  CompareWidgetState createState() => CompareWidgetState();
}

class CompareWidgetState extends State<CompareWidget> with SingleTickerProviderStateMixin {
  bool useShader = true;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x < size.width; x++) {
      for (double y = 0; y < size.height; y++) {
        ...
        col = Vector3(pow(col.x, 0.4545).toDouble(), pow(col.y, 0.4545).toDouble(), pow(col.z, 0.4545).toDouble());
        paint.color = Color.fromRGBO((col.x * 255).round(), (col.y * 255).round(), (col.z * 255).round(), 1);
